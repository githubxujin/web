...
Utils.generateChatroomSysMsg = function(data) {
  ...
  switch (data.attach.type) {
    ...
    case 'addTempMute':
      ...
      break;
    case 'removeTempMute':
      text = `${data.attach.toNick[0] ||
        data.attach.to[0]}被管理员解除临时禁言`;
      break;
    case 'addCommon':
      text = '管理员添加普通成员';
      break;
...
Utils.generateTeamSysmMsg = function(data) {
  var text;
  var nicks = this.getNickNames(data.attach.users);
  switch (data.attach.type) {
    case 'updateTeam':
...
Utils.getTeamUpdateInfo = function(msg) {
  ...
  const op = this.getNickNames(msg.attach.users).pop();
  if (team['name']) {
  ...
  // }
    ...
    text = `邀请他人权限为${
      team['inviteMode'] === 'all' ? '所有人' : '管理员'
    }`;
  } else if (team['updateTeamMode']) {
    text = `群资料修改权限为${
      team['updateTeamMode'] === 'all' ? '所有人' : '管理员'
    }`;
  } else if (team['beInviteMode']) {
    text = `被邀请人身份${
      team['beInviteMode'] === 'noVerify' ? '不需要验证' : '需要验证'
    }`;
  } else {
    text = '更新群信息';
  }
